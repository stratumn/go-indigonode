...
func NewTestNode(dir string, config cfg.ConfigSet) (*TestNode, error) {
	...
	if err := config.Save(confFile, 0600, cfg.SaveParams{Overwrite: true, Backup: true}); err != nil {
		return nil, err
	}
...
func NewTestNodeSet(dir string, n int, config cfg.ConfigSet) (TestNodeSet, error) {
	...
	for _, node := range nodes {
		...
		if err := node.conf.Save(confFile, 0600, cfg.SaveParams{Overwrite: true, Backup: true}); err != nil {
			return nil, err
		}
	}
