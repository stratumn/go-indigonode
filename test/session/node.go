...
import (
	...
	grpcapipb "github.com/stratumn/go-node/core/app/grpcapi/grpc"
	...
	managerpb "github.com/stratumn/go-node/core/manager/grpc"
...
func NewTestNode(dir string, config cfg.ConfigSet) (*TestNode, error) {
	...
	dhtConf.LevelDBPath = filepath.Join("data", "kaddht")
	...
	confFile := filepath.Join(dir, "stratumn_node.core.toml")
...
func (h *TestNode) Up(ctx context.Context) error {
	...
	cmd := exec.Command("stratumn-node", "up")
	cmd.Dir = h.dir
	cmd.Stdout = logf
	cmd.Stderr = logf
...
func NewTestNodeSet(dir string, n int, config cfg.ConfigSet) (TestNodeSet, error) {
	...
	for _, node := range nodes {
		confFile := filepath.Join(node.dir, "stratumn_node.core.toml")
